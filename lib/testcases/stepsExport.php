...
function init_args()
{
  ...
  {
    $args->export_filename = "testlink-steps-definition.xml";
  } 
...
function initializeGui(&$argsObj)
{
  ...
  $guiObj->page_title = lang_get('export_steps_difinition');
  $guiObj->do_it = 1;
...
*/
function doExport(&$db,$filename)
{
  $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
  $tables = tlObjectWithDB::getDBTables(array('steps_template'));
  $adodbXML = new ADODB_XML("1.0", "UTF-8");
  $sql = "/* $debugMsg */ SELECT step " .
         " FROM {$tables['steps_template']} ";
  
  $adodbXML->setRootTagName('steps');
  $adodbXML->setRowTagName('step');
